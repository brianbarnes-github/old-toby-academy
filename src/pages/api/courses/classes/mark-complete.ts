...
export const POST: APIRoute = async ({ request, cookies, redirect, locals }) => {
  ...
  const { data: cls, error } = await supabase
    .from('classes')
    .update({ closed_at: complete ? new Date().toISOString() : null })
    .eq('id', classId)
    .select('slug')
    .maybeSingle();
  ...
  await supabase.from('entries').insert({
    user_id: user.id,
    event_type: complete ? 'class_marked_complete' : 'class_reopened',
    details: { class_id: classId, course_slug: courseSlug, slug: cls.slug },
  });

  return redirect(`/courses/${courseSlug}/${cls.slug}/edit?ok=${complete ? 'completed' : 'reopened'}`);
};
